...
func Test_HandlerError_whe_endpoints_returns_internal_erros(t *testing.T) {
	assert := assert.New(t)
...
func Test_HandlerError_whe_endpoints_returns_domain_erros(t *testing.T) {
	assert := assert.New(t)

	errorString := "domain error"
	endpoint := func(w http.ResponseWriter, r *http.Request) (interface{}, int, error) {
		return nil, 0, errors.New(errorString)
	}
	...
	assert.Contains(res.Body.String(), errorString)
}

func Test_HandlerError_whe_endpoints_returns_obj_and_status(t *testing.T) {
	assert := assert.New(t)
